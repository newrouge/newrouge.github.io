...
## Kali (yeah you'd have guessed)
https://github.com/xct/kali-clean

https://i3wm.org/

## CopyQ (https://hluk.github.io/CopyQ/)
Particularly powerful when typing in a shell that doesn't support the same keyboard layout, or doesn't allow you to delete characters, or move in strings. You can prepare your commands in a buffer and send them when they are ready.
...
7. Want to check who has a terrible AD password? (username = password)
...
1. Find the overflow: just try with lots of characters until you get a crash, or a connection refused if you are remote
2. Find the offset:
...
4. Finding bad chars (with Immunity...):
...
And now send it to the vulnerable process:
...
I recommend using the [deREferecing]([url](https://github.com/danigargu/deREferencing)) plugin for IDA.
...
Once the breakpoint, or the crash is reached, it is possible to create a array of 255 characters from the stack's dump, by pressing '*' after clicking on ESP:
...
```bash
dump binary memory filtered.bin 0xffffd278 0xffffd378
```

where the addresses should be replaced by *esp esp+0xFF*


5. Genrate a shellcode
...
```
or you could also use -f python if you'd rather do things simply.

6. Find a ```jmp esp``` or equivalent
...
Although in my case my exploit during the exam worked on the first shot without putting any NOP (there is already a few in the msfvenom generated shellcode).
...
```
Setup iptables just like for meterpreter/reverse_tcp_allports, but make sure port 80 is queryiable, and start the netcat black hole:
...
```
Although, in reality its the powershell script that would most likely get blocked by the AV.
...
7. Want to get open ports on Windows?
...
8. Want to run IDA Free on a 32 bit Windows machine?
...
* gcc and required libs are on the machine: compile you exploit directly there
...
16. Exploiting an old timer with no modern way of downloading file? Use SMB Delivery1
...
```
Once the file system is mounted, you can read/write /etc/passwd and /etc/shadow. You can also add you public ssh key in /root/.ssh/authorized_keys.
...
I definitely had a lot of fun on the lab and during the exam too. I was expecting the challenges to be too "CTF" like, and based on riddles rather than seen-in-the-wild scenarios, but it wasn't really the case. In the end I feel like the OSCP has gotten easier with the inclusion of the AD set. I do think it brings some realism, as ADs are generally quite easy to root.

If you have any questions or comment, do not hesistate and reach me on discord.
