...
where the addresses should be replaced by *esp esp+0x10*


**Note:** the list of bad characters is 255 bytes long because 0x00 is generally omitted, as it marks the end of C-style terminated strings. For alignment reasons, it is usually easier to copy 0x10 bytes from the proram's memory, so there is an extra byte that does not come from the bad characters list.
...
```
or you could also use -f python if you'd rather do things simply. I use hex format as it is easier to translate to raw binary and can be used for other purposes.
